  ...
  <view>
	...
	<u-list class="list">
		...
		>
			...
			>
			<template slot="label">
				<span>{{"原料名称："+ item.maName}}</span><br/>
				<span>{{"原料库存："+ item.maInventory}}</span><br/>
...
<script>	
	import { listMaterial } from '@/api/system/material.js';
	export default {
		data() {
			return {
				maList: [],
		...
		methods: {
			getMaterial(){
				listMaterial().then(res => {
					this.maList = res.rows;
					console.log(this.maList);
					this.num = this.maList.length;
