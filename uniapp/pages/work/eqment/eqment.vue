  ...
  <view>
	...
	<u-list class="list">
		...
		>
			...
			<template slot="label">
				<span>{{"设备类别："+ getType(item.eqType)}}</span>
			</template>
			<template slot="label">
				<!-- <span>设备状态：</span> -->
				<u-tag v-if="item.eqStatus === 0" plain plainFill text="正常" type="success"  style="width: 60px;margin-top: 5px;"></u-tag>
				<u-tag v-if="item.eqStatus === 1" plain plainFill text="异常" type="warning" style="width: 60px;margin-top: 5px;" ></u-tag>
			</template>
			</u-cell>
			<u-popup :show="show" mode="center"  @close="close" overlayOpacity="0.1">
				<view style="width: 300px;height: 200px;">
					<span>{{"所在区域:"+item.arId}}</span><br/>
...
<script>	
	import { listEquipment } from '@/api/system/equipment.js';
	export default {
		data() {
			return {
				eqList: [],
		...
		methods: {
			getEquipment(){
				listEquipment().then(res => {
					this.eqList = res.rows;
					console.log(this.eqList);
					this.num = this.eqList.length;
					const filteredDevices = this.eqList.filter(item => item.eqStatus === 0)
					this.numOnline = filteredDevices.length;
					this.numFailed = this.num - this.numOnline;
				})
			},
			getType(eqtype){
